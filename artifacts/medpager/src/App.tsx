...
async function extractPdf(file: File){
  ...
  const pages: any[]=[];const max=Math.min(pdf.numPages,60);
  for(let p=1;p<=max;p++){const page=await pdf.getPage(p);const c=await page.getTextContent();pages.push({page:p,text:c.items.map((it: any)=>it.str).join(" ")});}
...
export default function App(){
  ...
  const[result,setResult]=useState<any>(SAMPLE);
  ...
  async function handleFile(file: File){
    ...
    try{
      const{pages:extracted}=await extractPdf(file);
      setPages(extracted);
...
function Header({demoMode,setDemoMode,language,setLanguage,stage}: any){
  return(
...
function Upload({fileRef,onFile,demoMode,language}: any){
  ...
  return(
    <section style={{paddingTop:60,animation:"rise .5s ease both"}}>
      ...
      <div onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}
        ...
        onDrop={(e: any)=>{e.preventDefault();const f=e.dataTransfer.files?.[0];if(f){setName(f.name);onFile(f);}}}
        role="button" tabIndex={0} className="b" onKeyDown={(e: any)=>e.key==="Enter"&&fileRef.current&&fileRef.current.click()}
        ...
        <input ref={fileRef} type="file" accept="application/pdf" style={{display:"none"}} onChange={(e: any)=>{const f=e.target.files?.[0];if(f){setName(f.name);onFile(f);}}}/>
      </div>
...
function Cite({page,quote,status,book}: any){
...
function Workspace({result,pages,tab,setTab,title,note,err,onReset,language}: any){
...
function Badge({children,color,bg}: any){return <span style={{fontSize:11,color,background:bg,padding:"3px 9px",borderRadius:999,fontFamily:"'JetBrains Mono',monospace"}}>{children}</span>;}

function Pager({result,book}: any){
  return(
    <div style={{animation:"rise .4s ease both"}}>
      <div style={{background:"#fff",border:`1px solid ${LINE}`,borderRadius:14,overflow:"hidden",boxShadow:"0 12px 30px rgba(19,32,28,.05)"}}>
        ...
        <div style={{padding:"10px 28px 28px"}}>
          {result.pager.map((b: any,idx: number)=>(
...
function Ask({result,book,pages,language}: any){
  const[active,setActive]=useState<number|null>(null);
  const[custom,setCustom]=useState("");
  const[customAnswer,setCustomAnswer]=useState<any>(null);
  ...
  async function handleAsk(){
    const q=custom.trim();
    if(!q)return;
    setActive(null);setCustomAnswer(null);setAskErr("");setLoading(true);
    try{
      ...
      const json=await resp.json();
      if(json.ok){setCustomAnswer({question:q,...json});}
      else{setAskErr(json.error||"Something went wrong. Try again.");}
    }catch(e){setAskErr(String(e));}
    setLoading(false);
  }

  function handleKeyDown(e: any){if(e.key==="Enter")handleAsk();}
  function selectPreset(i: number){setActive(i);setCustomAnswer(null);setAskErr("");setCustom("");}
  ...
  return(
    <div style={{animation:"rise .4s ease both"}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:22}}>
        <input value={custom} onChange={e=>setCustom(e.target.value)} onKeyDown={handleKeyDown} placeholder="Type your own question about the chapter…" style={{flex:1,border:`1px solid ${LINE}`,borderRadius:10,padding:"12px 14px",fontSize:14,fontFamily:"inherit",background:"#fff",color:INK}}/>
      ...
      {loading&&<div style={{display:"flex",alignItems:"center",gap:10,color:MUTED,fontSize:14,padding:"20px 0"}}><span style={{width:16,height:16,borderRadius:"50%",border:`2px solid ${HEAL}`,borderTopColor:"transparent",display:"inline-block",animation:"spin .7s linear infinite"}}/>Searching the chapter…</div>}
      ...
      {!showCustom&&!presetCur&&<div style={{textAlign:"center",color:MUTED,fontSize:14,padding:"40px 0"}}>Pick a preset question above or type your own to get a cited answer from the chapter.</div>}
    </div>
  );
}
...
function prepareQuiz(questions: any[]){
  return shuffleArray(questions).map(q=>{
    const n=q.options.length;
    const indices=shuffleArray([...Array(n).keys()]);
    return{...q,options:indices.map((i: number)=>q.options[i]),correct:indices.indexOf(q.correct)};
  });
}

function Quiz({result,book,pages,language}: any){
  const[quiz,setQuiz]=useState(()=>prepareQuiz(result.quiz));
  const[seenStems,setSeenStems]=useState(()=>result.quiz.map((q: any)=>q.q));
  ...
  function pick(i: number){if(chosen!=null)return;setChosen(i);if(i===q.correct)setScore(s=>s+1);}
  function next(){if(idx+1>=quiz.length){setFin(true);return;}setIdx(idx+1);setChosen(null);}
  function retake(){setQuiz(prepareQuiz(quiz));setIdx(0);setChosen(null);setScore(0);setFin(false);}
  ...
  async function generateNew(){
    ...
    try{
      ...
      if(json.ok&&Array.isArray(json.quiz)&&json.quiz.length>0){
        ...
        setQuiz(prepared);setSeenStems((s: any[])=>[...s,...json.quiz.map((q: any)=>q.q)]);
        setIdx(0);setChosen(null);setScore(0);setFin(false);
      }else{setGenErr(json.error||"Could not generate questions. Try again.");}
  ...
  return(
    <div style={{animation:"rise .4s ease both"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,fontSize:12,color:MUTED,fontFamily:"'JetBrains Mono',monospace",flexWrap:"wrap",gap:8}}>
        <span>Question {idx+1} of {quiz.length}</span><span>Score {score}</span>
      </div>
  ...
  return(
    <div style={{animation:"rise .4s ease both"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:14}}>
        {result.revision.map((c: any,i: number)=>{const o=flip[i];return(
